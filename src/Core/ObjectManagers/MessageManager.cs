...
namespace csammisrun.OscarLib
{
	...
	public class MessageManager : ISnacFamilyHandler
	{
		...
		/// <summary>
		/// ICBM code for a client/server message error or data
		/// </summary>
		private const int ICBM_MESSAGE_DATA = 0x000B;
		...
		/// <summary>
		/// A SNAC error code indicating that the user has denied sending messages to the intended recepient
		/// </summary>
		private const int ERROR_CODE_LOCAL_DENY = 0x0010;
		...
		//KSD-SYSTEMS - added at 01.02.2010 -> needed for a delay by deleting offline messages
		//KSD-SYSTEMS - added -> OfflineMessageKeepalive
		private readonly Timer offlineMessagesTimer;
		private bool offlineMessagesEndFlag = false;
		...
		internal MessageManager(Session parent)
		{
			...
			offlineMessagesTimer = new Timer(OfflineMessageKeepalive, null, Timeout.Infinite, Timeout.Infinite);
		}
		...
		public void ProcessIncomingPacket(DataPacket dp)
		{
			switch (dp.SNAC.FamilySubtypeID)
			{
				...
				case ICBM_UNDELIVERABLE_MESSAGE:
					...
					break;
				case ICBM_MESSAGE_DATA:
					//KSD-SYSTEMS - added at 03.12.2009 -> Contact Cancels a file sended by me
					//KSD-SYSTEMS - added -> ICBM_MESSAGE_DATA, ProcessClientServerData, ProcessClientServerDataChannelTwo
					ProcessClientServerData(dp);
					break;
			...
			if (messageStatuses.ContainsKey(dp.SNAC.RequestID))
			{
				...
				switch(errorcode)
				{
					case ERROR_CODE_BUDDYOFFLINE:
						...
						break;
					case ERROR_CODE_LOCAL_DENY:
						e.Status = MessageStatus.BuddyBlocked;
						break;
				}
		...
		/// <summary>
		/// Processes an error or data packet from server -- SNAC(04,0B)
		/// </summary>
		/// <param name="dp">A <see cref="DataPacket"/> object with a buffer containing SNAC(04,0B)</param>
		private void ProcessClientServerData(DataPacket dp)
		{
			// Pull apart the fixed part of the message
			byte[] message_cookie = dp.Data.ReadByteArray(8);
			Cookie cookie = Cookie.GetReceivedCookie(message_cookie);
			
			ushort channel = dp.Data.ReadUshort();

			byte screenNameLength = dp.Data.ReadByte();
			if (screenNameLength == 0) return;
			string ScreenName = dp.Data.ReadString(screenNameLength, Encoding.ASCII);
			...
			switch (channel)
			{
				case 0x0001:
					break;
				case 0x0002:
					ProcessClientServerDataChannelTwo(dp.Data, cookie, dp);
					break;
				default:
					break;
			}
		...
		private void GetChannelfourMessage(ByteStream stream, ref IM message)
		{
			...
			if (node != null)
			{
				...
				{
					offlineMessagesTimer.Change(750, 750);
					while (offlineMessagesEndFlag == false) { Thread.Sleep(0); }
					offlineMessagesEndFlag = false;
				...
				private void OfflineMessageKeepalive(object threadstate)
				{
					offlineMessagesTimer.Change(Timeout.Infinite, Timeout.Infinite);
					offlineMessagesEndFlag = true;
				}
				...
				private void AcceptIcbmOIM(OfflineIM message)
				{
					...
					{
						Logging.WriteString("Couldn't add offline Message to Queue: {0}", ex.Message);
			...
			public enum MessageStatus
			{
				...
				MessageTimeout,
				/// <summary>
				/// The user's permit/deny settings deny messaging the destination buddy
				/// </summary>
				BuddyBlocked,
			}
		}
